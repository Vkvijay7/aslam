...
const ScrollStack = ({
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          ...
          const jLastTranslateY = jLastTransform ? jLastTransform.translateY : 0;
          const jCardTop = getElementOffset(cardsRef.current[j]) - jLastTranslateY;
      ...
      // Hide cards until they approach their pin position
      let opacity = 1;
      if (i > 0) {
        const distanceToPin = cardTop - scrollTop - stackPositionPx - itemStackDistance * i;
        const fadeStartDistance = 200; // px before pinning to start fading in
        if (distanceToPin > fadeStartDistance) {
          opacity = 0;
        } else if (distanceToPin > 0) {
          opacity = 1 - (distanceToPin / fadeStartDistance);
        } else {
          opacity = 1;
        }
      }
  ...
  useLayoutEffect(() => {
    ...
    // In window scroll mode, we look up cards globally in the page context.
    ...
    // Calculate and cache layouts immediately
    calculateOffsets();
    window.addEventListener('resize', calculateOffsets);

    const scrollController = setupLenis();

    // Call layout transform immediately on render
    updateCardTransforms();
    ...
    return () => {
      ...
      if (scrollController && typeof scrollController.destroy === 'function') {
